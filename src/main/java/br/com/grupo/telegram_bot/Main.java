		...
		while (true) {
			...
			for (Update update : updates) {
				...
				if (update.message().text().equals("oi"))  {
					sendResponse = bot.execute(new SendMessage(update.message().chat().id(), "Oi Pessoa!"));
				}else{
					sendResponse = bot.execute(new SendMessage(update.message().chat().id(), "Sou um chat burro"));
				}
